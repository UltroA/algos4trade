...
class _GRUNet(nn.Module):
    def __init__(self, n_features: int, hidden: int = 32, num_layers: int = 1):
        super().__init__()
        self.gru = nn.GRU(input_size=n_features, hidden_size=hidden, num_layers=num_layers, batch_first=True)
...
class GRUPredictor(SingleAssetAlgorithm):
    name = "GRU Predictor"
    ...
    def fit(self, train_data: pd.DataFrame) -> "GRUPredictor":
        ...
        self.model = _GRUNet(n_features=X.shape[1], hidden=self.hidden, num_layers=self.num_layers)
